...
public:
    ...
    unordered_map<string, int> tempSubjectHoursPW;  // subject, hours per week
    unordered_map<string, int> subjectHoursPerWeek; // subject, hours per week
    int DAYS, HOURS;
    int fitness;

    Schedule(vector<Schedule> AllSchedules, int D, int H, string className, vector<pair<string, vector<string>>> subjects,
             unordered_map<string, int> subjectHoursPerWeek, unordered_map<string, string> choosenTeacher)
    {
        ...
        cout << "print subjects " << subjects.size() << endl;
        for (auto e : subjects)
        {
            cout << e.first << ":  ";
            for (auto f : e.second)
